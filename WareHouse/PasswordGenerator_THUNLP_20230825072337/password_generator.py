...
class PasswordGenerator:
    ...
    def generate_password(self, length, complexity, excluded_chars):
        ...
        if "uppercase" in complexity:
            ...
            password += secrets.SystemRandom().choice(self.uppercase_letters)
        if "lowercase" in complexity:
            chars += self.lowercase_letters
            password += secrets.SystemRandom().choice(self.lowercase_letters)
        if "numbers" in complexity:
            chars += self.numbers
            password += secrets.SystemRandom().choice(self.numbers)
        if "special_chars" in complexity:
            chars += self.special_chars
            password += secrets.SystemRandom().choice(self.special_chars)
        # Remove excluded characters
        chars = self.remove_excluded_chars(chars, excluded_chars)
        # Fill the remaining length with random characters
        remaining_length = length - len(password)
        if remaining_length > 0:
            password += "".join(secrets.SystemRandom().choice(chars) for _ in range(remaining_length))
        return password
