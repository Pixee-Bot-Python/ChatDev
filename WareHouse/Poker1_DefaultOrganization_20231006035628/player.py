...
class AIPlayer(Player):
    def make_decision(self, current_bet):
        if secrets.SystemRandom().random() < 0.05:  # 5% chance of making a random decision
            return secrets.SystemRandom().choice(["fold", "check", "bet", "call", "raise"])
        else:
            if current_bet > self.chips:
                return secrets.SystemRandom().choice(["call, fold"])
            elif current_bet == 0:
                return secrets.SystemRandom().choice(["check", "raise"])
            elif current_bet > self.previous_bet:
                return secrets.SystemRandom().choice(["fold", "call", "raise"])
            else:
                return secrets.SystemRandom().choice(["check", "call", "raise"])
    def get_raise_amount(self, current_bet):
